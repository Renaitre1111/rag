...
def get_classifier(dir_path='', device='cpu', model_type=None):
    with open(join(dir_path, 'args.pickle'), 'rb') as f:
        args_classifier = pickle.load(f)
    args_classifier.device = device
    target_model = model_type if model_type is not None else getattr(args_classifier, 'model_name', 'egnn')
    print(f"Loading classifier model: {target_model}")
    if target_model == 'egnn':
        args_classifier.model_name = 'egnn'
        classifier = main_qm9_prop.get_model(args_classifier)
    elif target_model == 'schnet':
        nf = getattr(args_classifier, 'nf', 128)
        n_interactions = getattr(args_classifier, 'n_interactions', 6)
        n_gaussians = getattr(args_classifier, 'n_gaussians', 50)
        cutoff = getattr(args_classifier, 'cutoff', 10.0)

        classifier = SchNet(in_node_nf=5, hidden_nf=nf, n_interactions=n_interactions, n_gaussians=n_gaussians, cutoff=cutoff, device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--generated_path', type=str, default='cond/gens/alpha_generated.txt')
    ...
    parser.add_argument('--generators_path', type=str, default='e3_diffusion/outputs/exp_cond_alpha')
    parser.add_argument('--classifiers_path', type=str, default='qm9/property_prediction/outputs/exp_class_alpha')
