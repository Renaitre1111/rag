...
def retrieve_dataloaders(cfg, DDP_flag=False):
    if 'qm9' in cfg.dataset:
        ...
        for split, dataset in datasets.items():
            # If DDP is enabled, create a DistributedSampler for the training set.
            if split == 'train' and DDP_flag:
                sampler = DistributedSampler(dataset, num_replicas=cfg.world_size, rank=cfg.rank, shuffle=True)
                # When a sampler is used, the shuffle argument in DataLoader must be False.
                shuffle = False
            else:
                sampler = None
                # Use original shuffle logic for validation/test sets or when DDP is off.
