...
def download_dataset_qm9(datadir, dataname, splits=None, calculate_thermo=True, exclude=True, cleanup=True):
    ...
    if not os.path.exists(gdb9_tar_data):
        logging.info(f"File {gdb9_tar_data} not found. Downloading...")
        try:
            urllib.request.urlretrieve(gdb9_url_data, filename=gdb9_tar_data)
            logging.info('GDB9 dataset downloaded successfully!')
        except Exception as e:
            logging.error(f"Failed to download GDB9 dataset: {e}")
            raise e
    else:
        logging.info(f"File {gdb9_tar_data} already exists. Skipping download.")

    # If splits are not specified, automatically generate them.
    if splits is None:
        splits = gen_splits_gdb9(gdb9dir, cleanup) # splits dict {'train': train_idx, 'valid': ..., 'test': ...}
...
def gen_splits_gdb9(gdb9dir, cleanup=True):
    ...
    gdb9_txt_excluded = join(gdb9dir, 'uncharacterized.txt') # 需要排除的分子列表
    if not os.path.exists(gdb9_txt_excluded):
        logging.info(f"File {gdb9_txt_excluded} not found. Downloading...")
        try:
            urllib.request.urlretrieve(gdb9_url_excluded, filename=gdb9_txt_excluded)
        except Exception as e:
            logging.error(f"Failed to download uncharacterized.txt: {e}")
            raise e
    else:
        logging.info(f"File {gdb9_txt_excluded} already exists. Skipping download.")
    ...
    with open(gdb9_txt_excluded) as f:
        ...
        excluded_strings = [line.split()[0]
                            for line in lines if len(line.split()) > 0] # 获取排除分子的ID

    excluded_idxs = [int(idx) - 1 for idx in excluded_strings if is_int(idx)] # str -> int
...
def get_thermo_dict(gdb9dir, cleanup=True):
    ...
    if not os.path.exists(gdb9_txt_thermo):
        logging.info(f"File {gdb9_txt_thermo} not found. Downloading...")
        try:
            urllib.request.urlretrieve(gdb9_url_thermo, filename=gdb9_txt_thermo)
        except Exception as e:
            logging.error(f"Failed to download atomref.txt: {e}")
            raise e
    else:
        logging.info(f"File {gdb9_txt_thermo} already exists. Skipping download.")
...
def get_unique_charges(charges):
    ...
    charge_counts = {z: np.zeros(len(charges), dtype=int)
                     for z in np.unique(charges)} # len(charges) is batch_size
