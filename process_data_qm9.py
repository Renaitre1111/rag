...
def write_qm9_to_seq(raw_path='qm9/data/temp',
                     write_path='data/',
                     split='train',
                     order_type='order',
                     remove_h=False,
    ...
    for i in tqdm(range(num_mol)):
        ...
        example_path = example_path = write_path + 'example/'
        os.makedirs(example_path, exist_ok=True)
        file_path = example_path + f'{split}_{i}.xyz'
        ...
        spherical_coords = np.array([["{:.3f}".format(value) for value in row] for row in spherical_coords])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Process QM9 data to sequential format.")
    
    parser.add_argument('--raw_path', type=str, default='qm9/temp/qm9/', help='Path to the folder containing .npz files')
    parser.add_argument('--write_path', type=str, default='data/', help='Output directory')
    parser.add_argument('--split', type=str, default='train', choices=['train', 'valid'], help='Dataset split to process')
    parser.add_argument('--order_type', type=str, default='order', help='Prefix for output filename')
    parser.add_argument('--remove_h', action='store_true', help='Whether to remove Hydrogen atoms')
    parser.add_argument('--sample', action='store_true', help='Debug mode: only process 5 molecules')
